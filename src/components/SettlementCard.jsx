 ...
 * @param month 조회 월
 */
function SettlementCard({ year, month }) {
  ...
  useEffect(() => {
    ...
    setError(false);
    api
      .get(`/api/workspaces/${currentWorkspaceId}/settlement`, { params: { year, month } })
      .then((res) => setSettlement(res.data ?? null))
      .catch((err) => {
        console.error('[Settlement] 로딩 실패:', err);
        setError(true);
        setSettlement(null);
      })
      .finally(() => setLoading(false));
  }, [currentWorkspaceId, members.length, year, month, refreshVersion]);
  ...
  return (
    <div className="bg-white rounded-3xl overflow-hidden shadow-sm">
      <div className="px-5 pt-5 pb-3">
        <p className="text-sm font-bold text-gray-900">이번 달 정산</p>
              ...
              <div key={member.userId} className="flex items-center justify-between">
                ...
                <span
                  className={`text-sm font-bold ${
                    member.netAmount > 0 ? 'text-blue-500' : member.netAmount < 0 ? 'text-red-500' : 'text-gray-400'
                  }`}
                >
          ...
          <div className="border-t border-gray-50 pt-3 space-y-2">
            {(settlement.transfers ?? []).length === 0 ? (
              ...
              settlement.transfers.map((transfer, index) => (
                <p key={`${transfer.fromUserId}-${transfer.toUserId}-${index}`} className="text-sm text-gray-700">
